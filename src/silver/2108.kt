...
fun main() {
    ...
    repeat(N) {
        ary.add(br.readLine().toInt())
    }

    val sum = (ary.sum().toDouble() / N).roundToInt()
    sb.append("$sum\n")
    ary.sort()
    val mid = ary[N /2]
    sb.append("$mid\n")

    val freqMap = ary.groupBy { it }.mapValues { it.value.size }
    val maxFreq = freqMap.maxOf { it.value }
    val modes = freqMap.filter { it.value == maxFreq }.keys.sorted()
    val mode = if (modes.size > 1) modes[1] else modes[0]
    sb.append("$mode\n")

    val range = ary.maxOrNull()!! - ary.minOrNull()!!
    sb.append("$range\n")
