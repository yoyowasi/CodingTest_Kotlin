...
// - 문제 | 집에서 시간을 보내던 오영식은 박성원의 부름을 받고 급히 달려왔다. 박성원이 캠프 때 쓸 N개의
// 랜선을 만들어야 하는데 너무 바빠서 영식이에게 도움을 청했다.
...
fun main() {
    ...
    val rope = LongArray(K)

    var maxVal: Long = 0
    for(i in 0 until K) {
        rope[i] = br.readLine().toLong()
        if(rope[i] > maxVal) maxVal = rope[i]
    }
